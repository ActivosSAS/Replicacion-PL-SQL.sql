create or replace PACKAGE BODY                 SEL.QB_APLICATION_JSEL0029 as
  --**********************************************************************************************************
    ...
    --******************************************************************************
    --****************************************************************
    --** MODIFICACION         : Se implementa insercion condicional en la tabla RHU.CANDIDATE_BULK_UPLOAD cuando la fuente es 
    --**                        'COMPUTRABAJO_MASIVO' y se recibe un identificador valido (vcedfciunombreteletiqueta).
    --** AUTOR MODIFICACIÓN   : JUFORERO
    --** FECHA MODIFICACIÓN   : 13/04/2025
    --****************************************************************
    --****************************************************************
    --** MODIFICACION         : Se implementa una validacion cuando llega por replicacion cuando la fuente es 
  ...
  */
    ...
    IS
      ...
      ciudad_count NUMBER := 0;
      
   -- Variables para validacion de ciudad del barrio (bar)
   l_pai_nombre   VARCHAR2(100) := vcpainombrebar;
   l_dpt_nombre   VARCHAR2(100) := vcdptnombrebar;
   l_ciu_nombre   VARCHAR2(100) := vcciunombrebar;

   -- Variables para validacion de ciudad de nacimiento 
   l_painac   VARCHAR2(100) := vcpainombrenac;
   l_dptnac   VARCHAR2(100) := vcdptnombrenac;
   l_ciunac   VARCHAR2(100) := vcciunombrenac;

   --  Variables para validacion de ciudad de residencia 
   l_paires   VARCHAR2(100) := vcpainombreres;
   l_dptres   VARCHAR2(100) := vcdptnombreres;
   l_ciures   VARCHAR2(100) := vcciunombreres;
   l_zon_nombre VARCHAR2(100) := vczonnombrebar;
    ...
    pb_seguimiento(
            ...
            'Masivo: ' ||
            ' | vceplnom1=' || vceplnom1 ||
            ' | vceplapell1=' || vceplapell1 ||
            ' | vctdctd=' || vctdctd ||
            ' | nmeplnd=' || nmeplnd ||
            ' | vcepl_email=' || vcepl_email ||
            ' | vcedociuexpedicionetiqueta=' || vcedociuexpedicionetiqueta ||
            ' | vcedopaiexpedicion=' || vcedopaiexpedicion ||
            ' | vcedodptexpedicion=' || vcedodptexpedicion ||
            ' | vcedociuexpedicion=' || vcedociuexpedicion ||
            ' | dtedofecexpedicion=' || TO_CHAR(dtedofecexpedicion, 'YYYY-MM-DD') ||
            ' | vceplsexo=' || vceplsexo ||
            ' | dteplfecnacim=' || TO_CHAR(dteplfecnacim, 'YYYY-MM-DD') ||
            ' | vcciunombrenacetiqueta=' || vcciunombrenacetiqueta ||
            ' | vcpainombrenac=' || vcpainombrenac ||
            ' | vcdptnombrenac=' || vcdptnombrenac ||
            ' | vcciunombrenac=' || vcciunombrenac ||
            ' | vcedfidiomanativo=' || vcedfidiomanativo ||
            ' | vcecvsigla=' || vcecvsigla ||
            ' | vcedfeplemailalterno=' || vcedfeplemailalterno ||
            ' | vccelular=' || vccelular ||
            ' | vcotrocelular=' || vcotrocelular ||
            ' | vctelefonocontacto=' || vctelefonocontacto ||
            ' | vcotrotelefonofijo=' || vcotrotelefonofijo ||
            ' | vcedfciunombreotfetiqueta=' || vcedfciunombreotfetiqueta ||
            ' | vcedfpainombreotf=' || vcedfpainombreotf ||
            ' | vcedfdptnombreotf=' || vcedfdptnombreotf ||
            ' | vcedfciunombreotf=' || vcedfciunombreotf ||
            ' | vcciunombreresetiqueta=' || vcciunombreresetiqueta ||
            ' | vcpainombreres=' || vcpainombreres ||
            ' | vcdptnombreres=' || vcdptnombreres ||
            ' | vcciunombreres=' || vcciunombreres ||
            ' | vcbarnombreetiqueta=' || vcbarnombreetiqueta ||
            ' | vcpainombrebar=' || vcpainombrebar ||
            ' | vcdptnombrebar=' || vcdptnombrebar ||
            ' | vcciunombrebar=' || vcciunombrebar ||
            ' | vczonnombrebar=' || vczonnombrebar ||
            ' | vcbarnombrebar=' || vcbarnombrebar ||
            ' | vcepldireccion=' || vcepldireccion ||
            ' | vceplnom2=' || vceplnom2 ||
            ' | vceplapell2=' || vceplapell2 ||
            ' | vcedfciunombreteletiqueta=' || vcedfciunombreteletiqueta ||
            ' | vcedfpainombretel=' || vcedfpainombretel ||
            ' | vcedfdptnombretel=' || vcedfdptnombretel ||
            ' | vcedfciunombretel=' || vcedfciunombretel ||
            ' | vceplgruposanguineo=' || vceplgruposanguineo ||
            ' | vceplfactorrh=' || vceplfactorrh ||
            ' | vcunidadorg=' || vcunidadorg ||
            ' | vcusuUsuario=' || vcusuUsuario ||
            ' | vcFuente=' || vcFuente
        );
    ...
    --****************************************************************
    --** AUTOR MODIFICACION   : JUFORERO
    --** FECHA MODIFICACION   : 07/05/2025
    --****************************************************************      
      IF vcFuente = 'COMPUTRABAJO_MASIVO' AND vcedfciunombreteletiqueta IS NOT NULL THEN
      SELECT COUNT(*)
      INTO ciudad_count
      FROM CIUDAD
      WHERE UPPER(pai_nombre) = UPPER(l_pai_nombre)
        AND UPPER(dpt_nombre) = UPPER(l_dpt_nombre)
        AND UPPER(ciu_nombre) = UPPER(l_ciu_nombre);

          IF ciudad_count = 0 THEN
             l_pai_nombre := 'COLOMBIA';
             l_dpt_nombre := 'CUNDINAMARCA';
             l_ciu_nombre := 'SANTAFE DE BOGOTA';
             
             l_painac   := 'COLOMBIA';
             l_dptnac   := 'CUNDINAMARCA';
             l_ciunac   := 'SANTAFE DE BOGOTA';

             l_paires   := 'COLOMBIA';
             l_dptres   := 'CUNDINAMARCA';
             l_ciures   := 'SANTAFE DE BOGOTA';
             l_zon_nombre := 'N.N.';
...
--      IF upper(vczonnombrebar) = 'CENTRO' THEN
        ...
        SELECT COUNT(*)
         ...
         AND upper(dpt_nombre) = upper(l_dpt_nombre)
         AND upper(ciu_nombre) = upper(l_ciu_nombre)
         AND upper(zon_nombre) = upper(l_zon_nombre);
        pb_seguimiento('DATBASHV', 'Zona'||zona_count);

         IF zona_count = 0 THEN
      BEGIN
        INSERT INTO zona (pai_nombre, dpt_nombre, ciu_nombre, zon_nombre, zon_codigo, zon_defecto) 
        VALUES (l_pai_nombre, l_dpt_nombre, l_ciu_nombre, l_zon_nombre, 0, 'N');
        COMMIT;
      EXCEPTION
        WHEN OTHERS THEN
      ...
      BEGIN
        ...
        VALUES (l_pai_nombre, l_dpt_nombre, l_ciu_nombre, l_zon_nombre, upper(vcbarnombrebar), 'N');
        COMMIT;
      EXCEPTION
        WHEN OTHERS THEN
        ...
        IF vcerror IS not null THEN raISe exnohacernada; END IF;
          ...
          BEGIN
              ...
              values(upper(vctdctd),nmeplnd ,upper(vceplapell1),upper(vceplapell2),upper(vceplnom1),upper(vceplnom2)
                      ,dteplfecnacim,upper(l_painac),upper(l_dptnac),upper(l_ciunac),upper(l_paires),upper(l_dptres)
                      ,upper(l_ciures)
                      ,upper(DECODE(l_zon_nombre,'NO APLICA','N.N.',l_zon_nombre))
          ...
          BEGIN
              ...
              set epl_apell1=upper(vceplapell1)
                  ...
                  ,epl_fecnacim=dteplfecnacim
                  ,pai_nombre_nac=upper(l_painac)
                  ,dpt_nombre_nac=upper(l_dptnac)
                  ,ciu_nombre_nac=upper(l_ciunac)
                  ,pai_nombre_res=upper(l_paires)
                  ,dpt_nombre_res=upper(l_dptres)
                  ,ciu_nombre_res=upper(l_ciures)
                  ,zon_nombre=upper(DECODE(l_zon_nombre,'NO APLICA','N.N.',l_zon_nombre))
...
create or replace PACKAGE             SEL.QB_APLICATION_JSEL0029 as
  --**********************************************************************************************************
    ...
    --****************************************************************************** 
    --****************************************************************
    --** MODIFICACIÃ¿N         : Se implementa inserciÃ³n condicional en la tabla RHU.CANDIDATE_BULK_UPLOAD cuando la fuente es 
    --**                        'COMPUTRABAJO_MASIVO' y se recibe un identificador vÃ¡lido (vcedfciunombreteletiqueta).
    --** AUTOR MODIFICACIÃ¿N   : JUFORERO
    --** FECHA MODIFICACIÃ¿N   : 13/04/2025
    --****************************************************************
    --****************************************************************
    --** MODIFICACIÃ¿N         : Se implementa una validacion cuando llega por replicacion cuando la fuente es 
    --**                        'COMPUTRABAJO_MASIVO', se valida Zona, Ciudad, Barrio
    --** AUTOR MODIFICACIÃ¿N   : JUFORERO
    --** FECHA MODIFICACIÃ¿N   : 07/05/2025
    --****************************************************************    
    ...
    ---------- retorna lista de valores tipos de identificaciÂ¿Â¿n-------------------
    PROCEDURE gettipodocumento(vcerror      OUT VARCHAR2,
                               vcconsulta   OUT refcursor);
